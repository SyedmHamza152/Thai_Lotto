...
// Mock helpers simulating your legacy 'api' wrapper and 'auth' session store modules.
// Replace these with your actual imported file settings later.
async function api(url: string, options: { method: string; body: any; auth: boolean }) {
...
const auth = {
  set: (token: string, userData: { user_id: number; public_id: string; username: string; phone: string; is_admin: boolean }) => {
  ...
  return (
    <div className="flex items-center justify-center min-h-screen p-5 bg-bg text-textCustom font-sans">
      ...
      >
        ...
        <label className="block text-[11px] font-semibold text-mutedCustom mb-1.5 uppercase tracking-[0.5px]">
          Phone number <span className="text-mutedCustom/60 normal-case">(optional)</span>
        </label>
        <input 
          className="w-full bg-surface2 border border-borderCustom text-textCustom px-3.5 py-2.5 rounded-xl text-sm outline-none transition-colors duration-150 focus:border-primaryCustom focus:shadow-[0_0_0_3px_rgba(168,85,247,0.25)]"
          name="phone" 
