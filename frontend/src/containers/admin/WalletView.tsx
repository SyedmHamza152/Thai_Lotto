...
export default function WalletView() {
  ...
  const handleLookupUser = async () => {
    ...
    try {
      const data = await api(`/api/admin/users/lookup?query=${encodeURIComponent(searchQuery.trim())}`);
      setUserData({
        memberId: data.public_id || '—',
        username: data.username || '—',
        phone: data.phone || '—',
        balance: data.balance !== undefined ? fmtRiyal(data.balance) : '—'
      });
    } catch (err: any) {
      setUserData(null);
  ...
  const handleApproveAdd = async () => {
    ...
    try {
      ...
      await api(`/api/admin/users/${memberIdToUse}/add-balance`, {
        method: 'POST',
  ...
  const handleDeductBalance = async () => {
    ...
    try {
      const memberIdToUse = userData ? userData.memberId : searchQuery.trim();
      await api(`/api/admin/users/${memberIdToUse}/deduct-balance`, {
        method: 'POST',
        body: { amount: Number(deductAmount) }
      });
